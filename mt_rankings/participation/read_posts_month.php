...
/**
 * This displays the rankings for the read posts for the current month.
 *
 * @package block_mt
...
$pagename = get_string ( 'mt_rankings:read_posts_month_page_name', 'block_mt' );
$pageurl = '/blocks/mt/mt_rankings/participation/read_posts_month.php';
$pageurlparams = array(
    'active' => $active
);
$logmtpage = true;
...
echo html_writer::tag ( 'h2', get_string ( 'mt_rankings:read_posts_month_desc', 'block_mt' ) );
...
$table->head = array (
    get_string ( 'mt_rankings:read_posts_month_rank', 'block_mt' ),
    get_string ( 'mt_rankings:read_posts_month_name', 'block_mt' ),
    get_string ( 'mt_rankings:read_posts_month_active', 'block_mt' ),
    get_string ( 'mt_rankings:read_posts_month_percent', 'block_mt' ),
);
...
$sql = "SELECT userid, period, courseid, percent_read
        FROM {block_mt_ranks_read_posts}
        WHERE period=:period AND courseid=:courseid AND percent_read > 0 AND period_type=:period_type
...
if ($DB->record_exists_sql($sql, $params )) {
    ...
    foreach ($studentdata as $student) {
        ...
        if (display_anonymous ( $student->userid, $courseid )) {
            $studentname = get_string ('mt_rankings:read_posts_month_anonymous', 'block_mt');
        } else {
            $studentname = get_string ('mt_rankings:read_posts_month_student_name', 'block_mt',
        ...
        $tablerow = new html_table_row ( array (
            ...
            number_format ( $student->percent_read, 2 )
        ) );
