...
/**
 * This displays the rankings for the total posts for the current month.
 *
 * @package block_mt
...
$pagename = get_string ( 'mt_rankings:total_posts_month_page_name', 'block_mt' );
$pageurl = '/blocks/mt/mt_rankings/participation/total_posts_month.php';
$pageurlparams = array(
    'active' => $active
);
$logmtpage = true;
...
echo html_writer::tag ( 'h2', get_string ( 'mt_rankings:total_posts_month_desc', 'block_mt' ) );
...
$table->head = array (
    get_string ( 'mt_rankings:total_posts_month_rank', 'block_mt' ),
    get_string ( 'mt_rankings:total_posts_month_name', 'block_mt' ),
    get_string ( 'mt_rankings:total_posts_month_active', 'block_mt' ),
    get_string ( 'mt_rankings:total_posts_month_number_posts', 'block_mt' ),
);
...
$sql = "SELECT userid, period, courseid, num_posts
        FROM {block_mt_ranks_num_posts}
        WHERE period=:period AND courseid=:courseid AND num_posts > 0 AND period_type=:period_type
...
if ($DB->record_exists_sql ( $sql, $params )) {
    ...
    foreach ($studentdata as $student) {
        ...
        if (display_anonymous ( $student->userid, $courseid )) {
            $studentname = get_string ('mt_rankings:total_posts_month_anonymous', 'block_mt');
        } else {
            $studentname = get_string ('mt_rankings:total_posts_month_student_name', 'block_mt',
