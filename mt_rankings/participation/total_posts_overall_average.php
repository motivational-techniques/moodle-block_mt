...
/**
 * This displays the rankings for the total posts overall
 *
 * @package block_mt
...
$pagename = get_string ( 'mt_rankings:total_posts_overall_page_name', 'block_mt' );
$pageurl = '/blocks/mt/mt_rankings/participation/total_posts_overall_average.php';
$pageurlparams = array(
    'active' => $active
);
$logmtpage = true;
...
echo html_writer::tag ( 'h2', get_string ( 'mt_rankings:total_posts_overall_desc', 'block_mt' ) );
...
$tableheader = array (
    get_string ( 'mt_rankings:total_posts_overall_rank', 'block_mt' ),
    get_string ( 'mt_rankings:total_posts_overall_name', 'block_mt' ),
    get_string ( 'mt_rankings:total_posts_overall_active', 'block_mt' ),
    get_string ( 'mt_rankings:total_posts_overall_average', 'block_mt' )
);
...
if ($active == 'false') {
    switch ($CFG->dbtype) {
        case DB_TYPE_POSTGRES :
            $sql = "SELECT  o_t.userid as userid, SUM(num_posts) AS total_num_posts, m_e.num_months,
        ...
        WHERE period > :period and o_t.courseid=:courseid
...
if ($DB->record_exists_sql ( $sql, $params )) {
    ...
    foreach ($studentlist as $id => $studentlist) {
        ...
        if (display_anonymous ( $studentlist->userid, $courseid )) {
            $studentname = get_string ( 'mt_rankings:total_posts_overall_anonymous', 'block_mt' );
        } else {
            $studentname = get_string ( 'mt_rankings:total_posts_overall_student_name', 'block_mt',
        ...
        $row = array (
            ...
            number_format ( $studentlist->average_posts_month, 2 )
        );
        $titletext = get_string ( 'mt_rankings:total_posts_overall_title_percent', 'block_mt', $studentlist );
        ...
        $monthdata = $DB->get_record ( 'block_mt_ranks_num_posts', $parameters );

        $cell = new html_table_cell ();
        $cell->attributes ['title'] = get_string ( 'mt_rankings:total_posts_overall_title_month',
            'block_mt', $previousmonths->format ( 'F Y' ) );
        $cell->attributes ['class'] = 'gradeColumn';

        if ($monthdata != null && $monthdata->num_posts != 0) {
            $cell->text = $monthdata->num_posts;
        } else {
            $cell->text = null;
        }
        $row [] = $cell;
        ...
        for ($monthcounter = 11; $monthcounter > 0; $monthcounter --) {
            ...
            $monthdata = $DB->get_record ( 'block_mt_ranks_num_posts', $monthlyparams );
