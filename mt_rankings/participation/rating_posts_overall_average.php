...
/**
 * This displays the rankings for the ratings posts overall.
 *
 * @package block_mt
...
$pagename = get_string ( 'mt_rankings:rating_posts_overall_page_name', 'block_mt' );
$pageurl = '/blocks/mt/mt_rankings/participation/rating_posts_overall_average.php';
$pageurlparams = array(
    'active' => $active
);
$logmtpage = true;
...
echo html_writer::tag ( 'h2', get_string ( 'mt_rankings:rating_posts_overall_desc', 'block_mt' ) );
...
$tableheader = array (
    get_string ( 'mt_rankings:rating_posts_overall_rank', 'block_mt' ),
    get_string ( 'mt_rankings:rating_posts_overall_name', 'block_mt' ),
    get_string ( 'mt_rankings:rating_posts_overall_active', 'block_mt' ),
...
if ($active == 'false') {
    switch ($CFG->dbtype) {
        case DB_TYPE_POSTGRES :
            $sql = "SELECT  o_t.userid as userid, rating_percent, m_e.num_months,
                rating_percent/m_e.num_months as average_posts_month, m_e.active
        FROM {block_mt_ranks_rating_posts} o_t
        ...
        GROUP BY o_t.userid, rating_percent, m_e.num_months, m_e.active
...
if ($DB->record_exists_sql ( $sql, $params )) {
    ...
    foreach ($studentlist as $id => $studentlist) {
        ...
        if (display_anonymous ( $studentlist->userid, $courseid )) {
            ...
            $studentname = get_string ( 'mt_rankings:rating_posts_overall_student_name', 'block_mt',
        ...
        $monthdata = $DB->get_record ( 'block_mt_ranks_rating_posts', $parameters );

        $cell = new html_table_cell ();
        $cell->attributes ['title'] = get_string ( 'mt_rankings:rating_posts_overall_title_month',
            'block_mt', $previousmonths->format ( 'F Y' ) );
        $cell->attributes ['class'] = 'gradeColumn';

        if ($monthdata != null && $monthdata->rating_percent != 0) {
            $cell->text = number_format ( $monthdata->rating_percent, 2 );
        } else {
            $cell->text = null;
        }
        $row [] = $cell;
        ...
        for ($monthcounter = 11; $monthcounter > 0; $monthcounter --) {
            ...
            $monthdata = $DB->get_record ( 'block_mt_ranks_rating_posts', $monthlyparams );
