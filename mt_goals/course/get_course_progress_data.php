 ...
 * @return array
 */
function block_mt_get_chart_progress_data(&$courseid, &$userid) {
    global $DB;
    ...
    foreach ($quizdata as $id => $quizdata) {
        $quizid = $quizdata->id;
        $data = new stdClass ();
        $data->name = block_mt_goals_get_quiz_name($quizdata->id );
        $data->goaltext = get_string('mt_goals:course_progress_to_start', 'block_mt');
        $data->progress = block_mt_goals_get_quiz_start_time_week($userid, $quizid);
        $data->goal = block_mt_goals_get_quiz_goal_time_start($userid, $quizid, $courseid);
        $rows [] = block_mt_goals_add_data_progress_graph($data );
    }
    ...
    foreach ($quizdata as $id => $quizdata) {
        ...
        $data->progress = block_mt_goals_get_quiz_completed_time_week($userid, $quizid);
        $data->goal = block_mt_goals_get_quiz_goal_time_completed($userid, $quizid, $courseid);
        $rows [] = block_mt_goals_add_data_progress_graph($data);
    }
    ...
    foreach ($assigndata as $id => $assigndata) {
        ...
        $data = new stdClass ();
        $data->name = block_mt_goals_get_assign_name($courseid, $assigndata->id);
        $data->goaltext = get_string('mt_goals:course_progress_to_complete', 'block_mt');
        $data->progress = block_mt_goals_get_assign_completed_time_week($userid, $assigndata->id);
        $data->goal = block_mt_goals_get_assign_goal_time_week($userid, $assignid, $courseid);
        $rows [] = block_mt_goals_add_data_progress_graph($data);
    }
