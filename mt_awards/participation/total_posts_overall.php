...
/**
 * This displays the awards list for the total posts overall
 *
 * @package block_mt
...
$pagename = get_string('mt_awards:total_posts_overall_page_name', 'block_mt');
$pageurl = '/blocks/mt/mt_awards/participation/total_posts_overall.php';
$pageurlparams = array(
    'active' => $active
);
$logmtpage = true;
...
$tableheader = array(
    get_string('mt_awards:total_posts_overall_name', 'block_mt'),
    get_string('mt_awards:total_posts_overall_active', 'block_mt'),
...
if ($DB->record_exists('block_mt_awards_count_all', $params)) {
    ...
    foreach ($studentlist as $id => $studentlist) {
        ...
        if (display_anonymous($studentlist->userid, $courseid)) {
            ...
            $studentname = get_string('mt_awards:total_posts_overall_student_name', 'block_mt',
        ...
        for ($monthcounter = 12; $monthcounter > 0; $monthcounter --) {
            ...
            $awardname = get_string('mt_awards:total_posts_overall_award_name', 'block_mt', $previousmonths->format('Y-n'));
            $titletext = get_string('mt_awards:total_posts_overall_title', 'block_mt', $previousmonths->format('F Y'));
