...
/**
 * This displays the awards list for the total posts in the current month
 *
 * @package block_mt
...
$pagename = get_string('mt_awards:total_posts_month_page_name', 'block_mt');
$pageurl = '/blocks/mt/mt_awards/participation/total_posts_month.php';
$pageurlparams = array(
    'active' => $active
);
$logmtpage = true;
...
$nextlevel = determine_next_level_total_posts_num($userid, $courseid, get_current_period());
...
$params->currenttext = get_string('mt_awards:total_posts_month_number_posts', 'block_mt');
$params->currentnum = $nextlevel['currentnum'];
$params->nexttext = get_string('mt_awards:total_posts_month_number_posts_next_level', 'block_mt');
...
$table->head = array(
    get_string('mt_awards:total_posts_month_name', 'block_mt'),
    get_string('mt_awards:total_posts_month_award', 'block_mt')
);
...
$awardname = get_string('mt_awards:total_posts_month_award_name', 'block_mt', date('Y-n'));
...
if ($DB->record_exists('block_mt_awards_user', $params)) {
    ...
    foreach ($studentdata as $id => $student) {
        ...
        if (display_anonymous($student->userid, $courseid)) {
            ...
            $studentname = get_string('mt_awards:total_posts_month_student_name', 'block_mt', block_mt_get_user_name($student->userid));
        }
